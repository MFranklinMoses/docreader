...
def auto(image):
    ...
    # cv2.imwrite("Scanned.jpg", cleaned_image)
    cv2.imshow('Scanned Image',cleaned_image)
    cv2.waitKey(0)
    if cv2.waitKey(25) & 0xFF == ord('q'):
        cv2.destroyAllWindows()
        cap.release()
...
def transformation(image):
    ...
    edges = cv2.Canny(threshold, 30, 170, apertureSize=7)
...
if __name__ == "__main__":
    ...
    if choice == 1:
        auto(proc_image)
        # Contour.findContourProc(proc_image)
    elif choice == 2:
        img = proc_image.copy()
